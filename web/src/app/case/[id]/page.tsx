  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-gov-gray-400" aria-label="Breadcrumb">
        ...
        <span className="text-gov-gray-600">Caso #{shortId}</span>
      </nav>

      {/* Header */}
      <div className="mt-4">
        <div className="flex items-start justify-between gap-4">
          <h1 className="text-2xl font-bold text-gov-gray-900 leading-snug">
            {caseData.title}
          </h1>
        ...
        <p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gov-gray-600">
          {periodLabel && (
            <span className="font-mono tabular-nums">{periodLabel}</span>
          )}
          {caseData.total_value_brl != null && caseData.total_value_brl > 0 && (
            <>
              {periodLabel && <span className="text-gov-gray-200">·</span>}
      ...
      {/* Summary */}
      {caseData.summary && (
        <section className="mt-6 rounded-lg border border-gov-gray-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-gov-gray-900 mb-2">Resumo do Caso</h2>
          <p className="text-sm text-gov-gray-600 leading-relaxed">{caseData.summary}</p>
        </section>
      )}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        {/* Signals */}
        <section className="rounded-lg border border-gov-gray-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-gov-gray-900 mb-3">
            Sinais{" "}
          ...
          {sortedSignals.length === 0 ? (
            <p className="text-sm text-gov-gray-400">Nenhum sinal associado</p>
          ) : (
            <ul className="space-y-2">
              {sortedSignals.map((signal) => (
                ...
                >
                  ...
                  />
                  <span className="font-mono tabular-nums text-xs text-gov-gray-600 w-8 shrink-0">
                    {signal.typology_code}
                  </span>
                  <span className="flex-1 truncate text-xs text-gov-gray-600" title={TYPOLOGY_LABELS[signal.typology_code] ?? signal.typology_name}>
                    {TYPOLOGY_LABELS[signal.typology_code] ?? signal.typology_name}
                  </span>
                  <span className="font-mono tabular-nums text-xs text-gov-gray-400 shrink-0">
        ...
        <section className="rounded-lg border border-gov-gray-200 bg-white p-4">
          ...
          {entityNames.length === 0 ? (
            <p className="text-sm text-gov-gray-400">Nenhuma entidade identificada</p>
          ) : (
            <ul className="space-y-2">
              {entityNames.map((name, i) => (
                <li key={i} className="flex items-center gap-2.5">
                  <EntityTypeIcon type="company" />
                  <span className="flex-1 truncate text-xs text-gov-gray-600" title={name}>
                    {name}
                  </span>
                </li>
              ))}
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-gov-blue-50 text-gov-blue-700 px-4 py-2 text-sm font-medium transition hover:opacity-80"
        >
        ...
        <Link
          href="/radar"
          className="inline-flex items-center gap-1.5 rounded-lg border border-gov-gray-200 bg-white px-4 py-2 text-sm font-medium text-gov-gray-600 transition hover:bg-gov-gray-50"
        >
