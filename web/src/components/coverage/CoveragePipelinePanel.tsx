...
function stepConfig(status: string) {
  ...
  return {
    dot: "bg-gov-gray-50 border border-gov-gray-200",
    ring: "ring-gov-gray-200",
    icon: Clock3,
    iconCls: "text-gov-gray-500",
    labelCls: "text-gov-gray-500",
  };
}
  ...
  if (loading || !summary) {
    return (
      <div className="rounded-xl border border-gov-gray-200 bg-white p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-gov-gray-500">Pipeline</p>
        <div className="mt-4 flex items-center gap-0">
          {STATIC_STEPS.map((step, index) => (
            <div key={step.code} className="flex items-center">
              <div className="flex flex-col items-center gap-1.5">
                <div className="h-8 w-8 animate-pulse rounded-full bg-gov-gray-50" />
                <div className="h-3 w-16 animate-pulse rounded bg-gov-gray-50" />
              </div>
              {index < STATIC_STEPS.length - 1 && (
                <div className="mx-1 h-px w-8 bg-gov-gray-200 sm:w-12" />
              )}
            </div>
          ))}
        </div>
          ...
          return (
            <div key={step.code} className="flex items-start">
              ...
              {index < steps.length - 1 && (
                <div className="mx-1 mt-4 h-px w-8 flex-shrink-0 bg-gov-gray-200 sm:w-10" />
              )}
            </div>
          );
        })}
