    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30">
      <div className="h-full w-full max-w-2xl overflow-y-auto bg-white p-4 shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-gov-gray-100 pb-3">
          <div>
            <h3 className="text-lg font-semibold text-gov-gray-900">Diagnostico da fonte</h3>
        ...
        {loading && (
          <p className="mt-4 text-sm text-gov-gray-600">Carregando preview da fonte...</p>
        )}
        {error && (
          ...
          <div className="mt-4 space-y-4">
            <div className="rounded-lg border border-gov-gray-200 bg-white p-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${coverageStatusColor(data.connector.worst_status)}`}>
            ...
            <div className="rounded-lg border border-gov-gray-200 p-3">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-4 text-sm text-gov-gray-700">
                  ...
                  <div key={job.job} className="rounded-md bg-gov-gray-50 p-2">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-gov-gray-600">{job.domain}</p>
                      </div>
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${coverageStatusColor(job.status)}`}>
                        {job.status}
                      </span>
                    </div>
                    ...
                    {job.latest_run && (
                      <div className="mt-2 rounded border border-gov-gray-200 bg-white p-2 text-xs text-gov-gray-700">
                        <p>
                          Execucao mais recente: <span className="font-medium">{job.latest_run.status}</span>
                          {job.latest_run.is_stuck ? " (travada)" : ""}
                        </p>
                        <p>
                          Itens: {job.latest_run.items_fetched} coletados / {job.latest_run.items_normalized} normalizados
                        </p>
                        <div className="mt-1">
                          <Link
            ...
            <div className="rounded-lg border border-gov-gray-200 p-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-gov-gray-500">Execucoes recentes</p>
              <div className="mt-2 space-y-2">
                {data.recent_runs.map((run) => (
                  <div key={run.id} className="rounded-md bg-gov-gray-50 p-2 text-xs text-gov-gray-700">
                    <p className="font-medium">
                      {run.status}
                      {run.is_stuck ? " (travada)" : ""}
                    </p>
                    <p>
                      Inicio: {run.started_at ? formatDateTime(run.started_at) : "Nao informado"}
                    </p>
                    <p>
                      Fim: {run.finished_at ? formatDateTime(run.finished_at) : "Em andamento"}
                    </p>
                  </div>
                ))}
              </div>
