...
function ProvenanceNode({ ref: r, depth, forceExpand }: ProvenanceNodeProps) {
  ...
  return (
    <li className="border-l border-gov-gray-200 pl-3">
      <div className="flex items-start gap-2 py-1.5">
        {hasChildren ? (
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="mt-0.5 shrink-0 text-gov-gray-600 hover:text-gov-gray-900"
        ...
        <RefIcon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gov-blue-700" />
        <div className="min-w-0 flex-1">
          <p className="text-sm text-gov-gray-900">{r.description}</p>
          <div className="mt-0.5 flex flex-wrap items-center gap-2">
            <span className="rounded bg-gov-gray-50 px-1 py-0.5 text-xs text-gov-gray-600">
              {config.label}
            </span>
            {r.url && (
              <a
                ...
                className="inline-flex items-center gap-0.5 text-xs text-gov-blue-700 underline hover:opacity-80"
              >
                fonte
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gov-gray-900">
        ...
        {defaultCollapsed && (
          <button
            ...
            className="text-xs text-gov-blue-700 hover:opacity-80"
          >
    ...
    return (
      <button
        ...
        className={cn(
          ...
          active ? "text-gov-blue-700" : "text-gov-gray-600 hover:text-gov-gray-900",
        )}
      >
        {label}
    ...
    <div className="mt-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={cn(
            "inline-flex items-center gap-1.5 rounded-md border border-gov-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gov-gray-600 transition hover:text-gov-gray-900",
      ...
      {/* Sort controls */}
      {items.length > 1 && (
        <div className="mt-2 flex items-center gap-3 text-xs text-gov-gray-600">
          <span>Ordenar:</span>
      ...
      {loading && items.length === 0 ? (
        <div className="mt-3 rounded-lg border border-gov-gray-200 bg-white p-4 text-sm text-gov-gray-600">
        ...
        <div className="mt-3 overflow-x-auto rounded-lg border border-gov-gray-200">
          <table className="min-w-full divide-y divide-gov-gray-200 text-sm">
            <thead className="bg-gov-gray-50">
              <tr>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gov-gray-600">
                  Data
                </th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gov-gray-600">
                  Descrição
                </th>
                <th className="px-3 py-2.5 text-right text-xs font-semibold text-gov-gray-600">
                  Valor
                </th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gov-gray-600">
                  Fonte
            ...
            <tbody className="divide-y divide-gov-gray-200 bg-white">
              {sorted.map((item) => (
                <tr key={item.event_id} className="hover:bg-gov-gray-50/50 transition-colors">
                  <td className="whitespace-nowrap px-3 py-2.5 font-mono tabular-nums text-xs text-gov-gray-600">
                    {item.occurred_at ? formatDate(item.occurred_at) : "—"}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-gov-gray-900">
                    <p className="font-medium">{item.description}</p>
                    {item.evidence_reason && (
                      <p className="mt-0.5 text-gov-gray-400">{item.evidence_reason}</p>
                    )}
                  </td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-right font-mono tabular-nums text-xs text-gov-gray-900">
                    {typeof item.value_brl === "number" ? formatBRL(item.value_brl) : "—"}
                  </td>
                  <td className="px-3 py-2.5">
                    <span className="rounded bg-gov-gray-50 px-1.5 py-0.5 font-mono text-xs text-gov-gray-600">
                  ...
                  <td className="px-3 py-2.5">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-0.5 text-xs text-gov-blue-700 hover:opacity-80"
      ...
      {/* Pagination */}
      {hasPagination && (
        <div className="mt-3 flex items-center justify-between text-xs text-gov-gray-600">
          <button
            type="button"
            disabled={offset === 0 || loading}
            onClick={() => onPageChange(Math.max(0, offset - limit))}
            className="rounded border border-gov-gray-200 bg-white px-2.5 py-1 text-gov-gray-600 disabled:cursor-not-allowed disabled:opacity-50 hover:text-gov-gray-900"
          >
            Anterior
          </button>
          <span>
