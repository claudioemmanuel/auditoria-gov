...
from openwatch_models.coverage import CoverageItem, CoverageMapItem, CoverageMapResponse
...
def _apply_signal_filters(
    ...
    *,
    ...
    if corruption_type or sphere:
        from openwatch_typologies.factor_metadata import get_typology_codes_for_filter
...
async def get_coverage_v2_summary(session: AsyncSession) -> dict:
    from openwatch_scheduler.schedule import BEAT_SCHEDULE
...
async def get_dossier_summary(
    ...
    from openwatch_models.orm import CaseItem, Typology, LegalViolationHypothesis
...
async def get_analytical_coverage(session: AsyncSession) -> list[dict]:
    ...
    for typo in get_all_typologies():
        ...
        from openwatch_typologies.factor_metadata import TYPOLOGY_LEGAL_METADATA
...
async def get_public_sources(session: AsyncSession) -> dict:
    ...
    from openwatch_connectors import ConnectorRegistry
    from openwatch_connectors.domain_guard import (
        DOMAIN_EXCEPTIONS,
        GOVERNMENT_TLDS,
        is_government_domain,
    )
    from openwatch_connectors.veracity import SOURCE_VERACITY_REGISTRY
